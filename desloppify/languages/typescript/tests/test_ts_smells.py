"""Tests for desloppify.languages.typescript.detectors.smells — TS/React code smell detection."""
...
def test_hardcoded_url_skips_module_constants(tmp_path):
    ...
    _write(
        tmp_path, "config.ts", "export const API_URL = 'https://api.example.com/v1';\n"
    )
    entries, _ = detect_smells(tmp_path)
...
def test_detect_dead_useeffect(tmp_path):
    ...
    _write(tmp_path, "bad.tsx", ("useEffect(() => {\n  // nothing here\n}, [dep]);\n"))
    entries, _ = detect_smells(tmp_path)
    ids = {e["id"] for e in entries}
...
def test_detect_monster_function(tmp_path):
    ...
    _write(tmp_path, "bad.ts", (f"function bigFunction() {{\n{body_lines}\n}}\n"))
    entries, _ = detect_smells(tmp_path)
    ids = {e["id"] for e in entries}
